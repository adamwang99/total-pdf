...
const PLACEHOLDER_AUTHORS = new Set(["Professional", "Digital Signature", ""]);

function getAuthorName(obj: any, currentDisplayName: string): string {
  const stored = (obj?.author ?? "Professional").trim() || "Professional";
  if (PLACEHOLDER_AUTHORS.has(stored)) return currentDisplayName || "Professional";
  return stored;
}
...
function isReplyAuthoredByCurrentUser(
  ...
  const stored = (obj?.author ?? "").trim() || "Professional";
  ...
  const resolvedMine = PLACEHOLDER_AUTHORS.has(
    ...
    ? "Professional"
    : (currentDisplayName ?? "").trim();
  const resolvedStored = PLACEHOLDER_AUTHORS.has(stored) ? "Professional" : stored;
  // Both are guest/anonymous → same unauthenticated user, allow editing
  if (resolvedStored === "Professional" && resolvedMine === "Professional") return true;
